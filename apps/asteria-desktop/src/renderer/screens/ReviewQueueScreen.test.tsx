...
describe("ReviewQueueScreen", () => {
  ...
  it("toggles guide visibility and resets layers", async () => {
    (globalThis as typeof globalThis & { asteria?: unknown }).asteria = {
      ipc: {
        "asteria:fetch-review-queue": vi.fn().mockResolvedValue(
          ok({
    ...
    const user = userEvent.setup();
    render(<ReviewQueueScreen runId="run-6" runDir="/tmp/runs/run-6" />);

    expect(await screen.findByText(/Review Queue/i)).toBeInTheDocument();

    const guidesToggle = screen.getByLabelText(/toggle guides/i) as globalThis.HTMLInputElement;
    expect(guidesToggle.checked).toBe(true);
    await user.click(guidesToggle);
    expect(guidesToggle.checked).toBe(false);

    const snappingToggle = screen.getByLabelText(/toggle snapping/i) as globalThis.HTMLInputElement;
    expect(snappingToggle.checked).toBe(true);
    await user.click(snappingToggle);
    expect(snappingToggle.checked).toBe(false);

    const baselineLayer = screen.getByLabelText(
      /baseline grid guide layer/i
    ) as globalThis.HTMLInputElement;
    await user.click(baselineLayer);
    expect(baselineLayer.checked).toBe(false);

    await user.click(screen.getByRole("button", { name: /reset guides visibility/i }));
    expect(baselineLayer.checked).toBe(true);
  });
});
