...
const ARTIFACTS_DIR = path.join(ROOT, "artifacts", "preflight");
...
const runPreflight = async () => {
  ...
  const commands = [];
  const commandSteps = [
    { id: "lint", label: "Lint", command: "pnpm", args: ["nx", "run", "asteria-desktop:lint"] },
    { id: "typecheck", label: "Typecheck", command: "pnpm", args: ["nx", "run", "asteria-desktop:typecheck"] },
    {
      id: "build-main",
      label: "Build Main",
      command: "pnpm",
    ...
    { id: "test", label: "Tests", command: "pnpm", args: ["nx", "run", "asteria-desktop:test"] },
    { id: "golden", label: "Golden", command: "pnpm", args: ["nx", "run", "asteria-desktop:golden"], skipEnv: "PREFLIGHT_SKIP_GOLDEN" },
    { id: "e2e", label: "E2E", command: "pnpm", args: ["nx", "run", "asteria-desktop:e2e"], skipEnv: "PREFLIGHT_SKIP_E2E" },
  ];
  ...
  if (process.env.PREFLIGHT_SKIP_PIPELINE !== "1") {
    ...
    if (!corpusPath) {
      ...
      const outputDir =
        process.env.ASTERIA_OUTPUT_DIR ?? path.join(ROOT, "pipeline-results");
    ...
    warnings.push("Determinism run skipped via PREFLIGHT_SKIP_PIPELINE");
  }
  ...
  await writeJson(path.join(ARTIFACTS_DIR, "preflight-report.json"), report);
  ...
  await writeText(path.join(ARTIFACTS_DIR, "preflight-report.md"), markdown);
